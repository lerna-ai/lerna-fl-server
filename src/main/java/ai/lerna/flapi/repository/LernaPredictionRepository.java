...
public interface LernaPredictionRepository extends JpaRepository<LernaPrediction, Long> {
	...
	@Query(value = "SELECT COUNT(DISTINCT device_id) FROM (SELECT device_id FROM lerna_predictions p, lerna_ml ml, lerna_app a WHERE ml.id = p.ml_id AND ml.app_id = a.id AND a.user_id = :userId AND a.id = :appId AND timestamp >= current_date - interval '7 days' UNION SELECT device_id FROM ml_history_datapoint hd, ml_history h, lerna_ml ml, lerna_app a WHERE hd.history_id = h.id AND ml.id = h.ml_id AND ml.app_id = a.id AND a.user_id = :userId AND a.id = :appId AND timestamp >= current_date - interval '7 days' UNION SELECT s.device_id FROM success s, lerna_ml ml, lerna_app a WHERE ml.id = s.ml_id AND ml.app_id = a.id AND a.user_id = :userId AND a.id = :appId AND timestamp >= current_date - interval '7 days') AS device_id", nativeQuery = true)
	long getTotalDevicesLastWeek(long userId, long appId);

	@Query(value = "SELECT COUNT(DISTINCT device_id) FROM (SELECT device_id FROM lerna_predictions p, lerna_ml ml, lerna_app a WHERE ml.id = p.ml_id AND ml.app_id = a.id AND a.user_id = :userId AND a.id = :appId AND timestamp < current_date - interval '7 days' AND timestamp >= current_date - interval '14 days' UNION SELECT device_id FROM ml_history_datapoint hd, ml_history h, lerna_ml ml, lerna_app a WHERE hd.history_id = h.id AND ml.id = h.ml_id AND ml.app_id = a.id AND a.user_id = :userId AND a.id = :appId AND timestamp < current_date - interval '7 days' AND timestamp >= current_date - interval '14 days' UNION SELECT s.device_id FROM success s, lerna_ml ml, lerna_app a WHERE ml.id = s.ml_id AND ml.app_id = a.id AND a.user_id = :userId AND a.id = :appId AND timestamp < current_date - interval '7 days' AND timestamp >= current_date - interval '14 days') AS device_id", nativeQuery = true)
	long getTotalDevicesPreviousWeek(long userId, long appId);
}
